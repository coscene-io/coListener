...
AgiAction::AgiAction() : curl_(nullptr), headers_(nullptr), dev_null_(nullptr) {
...
bool AgiAction::execute(const std::vector<MessageCache>& messages) {
    ...
    for (const auto& cache_item : messages) {
        ...
        try {
            item["msg"] = nlohmann::json::parse(nlohmann::json::parse(cache_item.msg)["data"].get<std::string>());
    ...
    try {
        ...
        const auto message_length = json_str.length();
        COLOG_DEBUG("Sending request to endpoint: %s", endpoint_.c_str());
        COLOG_DEBUG("Request payload size: %zu bytes", message_length);
        ...
        curl_easy_setopt(curl_, CURLOPT_POSTFIELDSIZE, message_length);
